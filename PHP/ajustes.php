...
<body>
        ...
        <div class="container mt-5">
            <div class="row fila">
                <div class="col-lg-6 menuAjustes">
                    <div class="tituloAjustes">
                        Ajustes
                    </div>
                    <div class="menus">
                            <ul id='alertas'> <span id='icoAlertas'></span> - </span><span id='listaAlertas'> Alertas </span>
                                <li><span id='gestionAlertas'>Gestionar Alertas</span></li>
                            </ul>
                            <ul id='seguridad'> <span id='icoSeguridad'>-</span><span id='listaSeguridad'> Seguridad y claves</span>
                                <li><span id='modificar'>Modificar Contraseña</span></li>
                            </ul>
                            <ul id='datos'> <span id='icoDatos'>-</span><span id='listaDatos'> Datos personales </span>
                                <li><span id='MostrarDatos'>Mostrar Datos</span></li>
                            </ul>
                    </div>
                </div>
                <div class="col mt-5" id='formularioAjustes'>
                        <?php
                         $id_cliente = $pdo->query('select Id_cliente FROM clientes where DNI LIKE "'.$_COOKIE['usuario'].'"')->fetchColumn(); 
                            if(isset($_POST['crearAlerta'])){
                                $miSaldo = $pdo->query("SELECT Saldo from clientes where DNI like '".$_COOKIE['usuario']."'")->fetchColumn();
                                $existeAlerta = $pdo->query("SELECT alertas from clientes where DNI like '".$_COOKIE['usuario']."'")->fetchColumn();

                                if($_POST['campoCantidad'] > $miSaldo){
                                    echo "<script>
                                    ...
                                    Toast.fire({
                                        icon: 'error',
                                        title: ' El saldo introducido es mayor del que dispones.'
                                    })
                                 
                                    </script>";
                                    // echo "<span class='avisos'><b>Aviso:</b> El saldo introducido es mayor del que dispones. </span>";                               
                                }else{
                                   if($existeAlerta == "1"){
                                    echo "<script>
                                    ...
                                    Toast.fire({
                                        icon: 'error',
                                        title: 'Ya dispones de una alerta.<br> Para poder crear una alerta nueva debes eliminar la anterior . '
                                    })
                                 
                                    </script>";
                                        // echo "<span class='avisos'> <b>Aviso:</b> Ya dispones de una alerta.<br> Para poder crear una alerta nueva debes eliminar la anterior . </span>";
                                   }else{
                                    $statement = $pdo->prepare("INSERT INTO alertas (id_cliente, cantidadLimite) VALUES(:id_cliente, :cantidadLimite)");
                                    $statement->bindValue(":id_cliente", $id_cliente);
                                    $statement->bindValue(":cantidadLimite", $_POST['campoCantidad']);
                                    $statement->execute();

                                    $statement1 = $pdo->prepare("UPDATE clientes SET alertas = '1'");
                                    $statement1->execute();
                                    echo "<script>
                                    ...
                                    Toast.fire({
                                        icon: 'success',
                                        title: 'Alerta creada con exito. Limite (".$_POST['campoCantidad'].",00€) '
                                    })
                                 
                                    </script>";
                                    // echo "<span class='avisos'> Alerta creada con exito. Limite (".$_POST['campoCantidad'].",00€) </span>";
                                   }
                                }
                                echo "<button class='volver' id='volver'> Volver </button>";
                            }
                            if(isset($_POST['eliminarAlerta'])){
                                $statement2 = $pdo->prepare("UPDATE clientes SET alertas = '0'");
                                $statement2->execute();

                                $statement3 = $pdo->prepare("DELETE FROM alertas WHERE id_cliente LIKE $id_cliente");
                                $statement3->execute();
                                ...
                                Toast.fire({
                                    icon: 'success',
                                    title: 'Alerta eliminada con exito'
                                })
                            ...
                            if(isset($_POST['datosPersonales'])){
                                try {
                                    $statement = $pdo->prepare("SELECT Correo, Nombre, Apellidos, DNI, Telefono ,Localidad, Provincia FROM clientes WHERE DNI = '".$_COOKIE['usuario']."'");
                                    $statement->execute();
                                    while ($result = $statement->fetch()) {
                                        echo "<h3 class='tituloDatos'>Datos Personales</h3>";

                                        echo "<table>
                                              <tr><th>Nombre</th><td>".$result['Nombre']." ".$result['Apellidos']."</td></tr>
                                              <tr><th>Identificacion oficial </th><td>".$result['DNI']."</td></tr>
                                              <tr><th>Domicilio </th><td>".$result['Localidad'].", ".$result['Provincia']."</td></tr>
                                              <tr><th>Datos de Contacto </th><td></td></tr>
                                              <tr><th class='contact'>Teléfono móvil: </th><td>".$result['Telefono']."</td></tr>
                                              <tr><th class='contact'>Teléfono Fijo: </th><td>".$result['Telefono']."</td></tr>
                                              <tr><th class='contact'>Correo electrónico: </th><td>".$result['Correo']."</td></tr>
                                              </table>";
                                    }	
                            ...
                            if(isset($_POST['cambiarPassword'])){
                                try {
                                    $statement = $pdo->prepare("UPDATE clientes SET Contraseña  = '".$_POST['passNueva']."' WHERE DNI LIKE '".$_COOKIE['usuario']."'");
                                    $statement->execute();
                                ...
                                echo "<button class='volverPass'><a href='./pagina_principal.php'>Volver</a></button>";
                            }
            ...
            <div class="row">
                ...
                <div class="resultadosAjustes" id='resultadosAjustes'>
